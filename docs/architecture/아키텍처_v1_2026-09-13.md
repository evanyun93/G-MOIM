...
erDiagram
    ...
    image {
        ...
        int byte_size "쿼터 집계. 변형본 포함 여부는 4.1.4에서"
        int width
        int height
        text format
...
| 항목 | 설계 노드 | 영향 받는 그림 |
|---|---|---|
| 이미지 변환 위치 (업로드 시 / 엣지 / 별도 서비스) | 4.1.4 | 1, 2 |
| `image.byte_size`에 변형본 용량 포함 여부 | 4.1.4 | 4.4 |
